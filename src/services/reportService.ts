...
export async function collectClaimReportData(claimId: string): Promise<ClaimReportData | null> {
  const [claim, risk, providers, cases] = await Promise.all([
    getClaim(claimId),
    getRiskResult(claimId),
    getProviders(),
    getInvestigations(),
  ]);
  if (!claim || !risk) return null;
  ...
  return {
    ...
    investigation: cases.find((c) => c.claim_id === claim.claim_id),
    traceability: {
      claim_id: claim.claim_id,
      model_run_id: "RUN-4821",
      generated_at: risk.explanation.generated_at,
      evidence_used: risk.evidence.map((e) => e.factor),
    },
  };
}
...
export async function collectProviderReportData(providerId: string): Promise<ProviderReportData | null> {
  ...
  return {
    ...
    risk_factors: topRiskFactorsChart,
    related_cases: cases.filter((c) => c.provider_id === provider.provider_id),
  };
}
...
function renderClaim(
  ...
  coverFor(
    ...
    { score: risk.risk_score, level: risk.risk_level },
  );

  doc.section("Section 1 — Risk Summary");
  doc.riskPanel(risk.risk_score, risk.risk_level, risk.prediction);
  doc.callout("Threshold note", THRESHOLD_NOTE, "neutral");
  ...
  doc.bullets(
    risk.evidence.map(
      (e) =>
        `${e.factor}: observed ${
          e.unit === "currency" ? currency(e.provider_value) : num(e.provider_value)
        } vs peer ${e.unit === "currency" ? currency(e.peer_value) : num(e.peer_value)} (${e.difference}).`,
    ),
  );
  ...
  if (trace) {
    ...
    doc.keyValues([
      ...
      { label: "Explainer", value: risk.explanation.model ?? "Not available" },
